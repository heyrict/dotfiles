			...
			local conf = {
				...
				hooks = {
					ProofRead = function(gp, params)
						local template = "你是一位资深研究员，以下是你需要修改的内容:\n\n"
							.. "```{{filetype}}\n{{selection}}\n```\n\n"
					...
					Translate = function(gp, params)
						local template = "你是一名专业的翻译助手，你的工作是进行中英互译。以下是文件 {{filename}} 中的一段话:\n\n"
							.. "```{{filetype}}\n{{selection}}\n```\n\n"
							.. "你在翻译过程中需要采用精准且符合本土化语境的词汇。请确保你的翻译既保留了原文的意图，又能够适应目标语言的文化和习惯。在翻译过程中，对于专业术语、成语或文化特定的表达，务必寻找最恰当的对应词汇，同时在保持原文风格的基础上，尽量做到语言地道、通顺。请注意，翻译不仅是对文字的直接转换，更是一种文化和信息的传递。请翻译以上文章。"
						local agent = gp.get_chat_agent()
						gp.Prompt(params, gp.Target.vnew, agent, template)
					end,
				},
